...
Amazon Quick Suite 데모를 위한 현실적인 스마트폰 제조사(Nova 브랜드) 데이터셋을 생성하는 Python 기반 시스템입니다. 판매, 고객 트랜잭션, 마케팅 캠페인, 소셜 미디어 포스트, 제품 리뷰 등 실제 비즈니스 시나리오를 시뮬레이션하는 정형 데이터(CSV)와 비정형 데이터(JSON)를 생성합니다.
